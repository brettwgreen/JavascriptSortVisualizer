...
var JsArrayViewModel = function() {
  ...
  self.doTheBubble = function(i) {
    ...
    if (i+1 <= self.data().length-1) {
      self.data()[i+1].currentStart(true);
      self.data()[i+1].inspecting(true);
      setTimeout(self.doTheBubble, self.delay(), i+1);
    }
    else {
  ...
  self.startBubbleSort = function() {
    ...
    setTimeout(self.doTheBubble, self.delay(), self._startingIndex);
  };

  self.completeBubbleSortLoop = function() {
